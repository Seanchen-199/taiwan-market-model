...
if new_frames:
    ...
    for days_back in range(7):
        ...
        url = f"https://www.twse.com.tw/rwd/zh/fund/T86?response=json&date={d.strftime('%Y%m%d')}&selectType=ALLBUT0999"
        try:
            ...
            if data.get('stat') == 'OK' and data.get('data'):
                ...
                for r in reversed(rows):
                    if '合計' in str(r[0]):
                        total_row = r; break
                if not total_row:
                    foreign_net = sum(pn(r[4]) for r in rows if len(r)>4)
                    invest_net  = sum(pn(r[7]) for r in rows if len(r)>7)
                    dealer_net  = sum(pn(r[8]) for r in rows if len(r)>8)
                else:
                    foreign_net = pn(total_row[4]) if len(total_row)>4 else 0
                    invest_net  = pn(total_row[7]) if len(total_row)>7 else 0
                    dealer_net  = pn(total_row[8]) if len(total_row)>8 else 0
                ...
                print(f"  ✅ 三大法人 {d}：外資 {foreign_net/100_000:.1f}億")
                break
        except Exception as e:
            print(f"  [警告] 三大法人 {d}: {e}")
        time.sleep(0.3)
